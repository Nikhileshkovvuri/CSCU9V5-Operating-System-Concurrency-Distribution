...
public class Auctioneer {
	...
	public Auctioneer(int inpor, int outpor, int givenRounds) {

		rnd = new Random();
		in_port = inpor;
		out_port = outpor;
		RandomAccessFile current_bid;
		int the_bid = 100; // >>> THis is used to set the initial price- done

		System.out.println("Auctioneer: " + in_port + " of distributed lottery is active ....");
		// roundDone is initialized below
		int roundDone;
		// AUCTIONEER:
		...
		/*
		 * the for loop here is used to continue the number of rounds completed in the
		 * variable roundDone until it has reached the number of rounds given in the
		 * parameter givenRounds.
		 */
		for (roundDone = 0; roundDone < givenRounds; roundDone++) {
			...
			try {
				...
				// below the file bid.txt is created

				current_bid = new RandomAccessFile("bid.txt", "rw");
				current_bid.seek(0);
				// the if statement is used to read the initial bid before the first loop has
				// started
				if (roundDone != 1) {
					the_bid = current_bid.readInt();
					current_bid.seek(0);
				}
				// Here the initial auction bid is entered into the bid.txt and the bid.txt is
				// closed
			...
			try {
				...
				// >>>
				// **** Forward the the token through a socket that connects to the
				// out_port-completed
				/*
				...
				// socket-based server mechanism- completed
				if (roundDone == 0) {
	...
	public static void main(String args[]) {
		...
		if (args.length != 3) {
			System.out.print("Usage: Auctioneer [port number] [forward port number] [Rounds]");
			System.exit(1);
		}
		...
		// the roundGiven parameter has been added here.
